...
Next, M7a: registry access, detection, deliberate recreate, carrying forward from this milestone a closed step-detail vocabulary (currently fixed text), a started-marker in the agent's deployment ledger, refusing a request whose deadline implies host/server clock skew, and plan-time inspection of live host configuration beyond the stored precondition identity.
