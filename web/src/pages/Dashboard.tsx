...
function Fleet({ org, mode }: { org: string; mode: 'containers' | 'endpoints' }) {
  ...
  return <>
    ...
    {endpoints.state === 'ready' && (mode === 'containers' ? (host ? [host] : []) : hosts).map((e) => <section className="panel" key={e.id}>
      ...
      {mode === 'containers' && <HostContainers org={org} endpoint={e.id} hostName={e.name} active={e.state === 'active'} />}
    </section>)}
...
function HostContainers({ org, endpoint, hostName, active }: { org: string; endpoint: string; hostName: string; active: boolean }) {
