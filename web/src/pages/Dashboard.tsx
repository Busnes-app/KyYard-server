...
export function Dashboard({ mode = 'containers' }: { mode?: 'containers' | 'endpoints' }) {
  ...
  return <div className="ky-page">
    ...
    <p>{mode === 'containers' ? 'Select a Docker host to browse and manage its containers.' : 'Connect Docker hosts, review enrollment, and inspect their resources.'}</p>
    ...
    {org && <>
      <div className="ky-toolbar">{orgs.length > 1 && <><label htmlFor="fleet-org">Access scope</label><select id="fleet-org" value={org.id} onChange={(e) => setSelected(e.target.value)} style={{ width: 'auto' }}>{orgs.map((o) => <option key={o.id} value={o.id}>{o.name}</option>)}</select></>}<Link className="btn btn-secondary" to={orgPath(org.id)}>Environments & add host</Link></div>
...
function Fleet({ org, mode, exec }: { org: string; mode: 'containers' | 'endpoints'; exec: boolean }) {
  ...
  const host = hosts.find((e) => e.id === selectedHost) ?? hosts[0];
  return <>
    <div className="ky-toolbar">
      {mode === 'containers' && hosts.length > 1 && <><label htmlFor="fleet-host">Docker host</label><select id="fleet-host" value={host?.id ?? ''} onChange={(event) => setSelectedHost(event.target.value)}>{hosts.map((e) => <option key={e.id} value={e.id}>{e.name}</option>)}</select></>}
    ...
    {endpoints.state === 'ready' && (mode === 'containers' ? (host ? [host] : []) : hosts).map((e) => <section className="panel" key={e.id}>
      ...
      {mode === 'containers' && <HostContainers org={org} endpoint={e.id} hostName={e.name} active={e.state === 'active'} exec={exec} />}
    </section>)}
...
function HostContainers({ org, endpoint, hostName, active, exec }: { org: string; endpoint: string; hostName: string; active: boolean; exec: boolean }) {
