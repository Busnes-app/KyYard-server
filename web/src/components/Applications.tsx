  ...
  return <section className="panel" aria-label="Applications">
    ...
    {drafts.state === 'ready' && <ul className="ky-list">
      ...
      {pagination.rows.map((draft) => <li key={draft.id}>
        ...
        {selected === draft.id && <><RevisionView key={`${draft.id}/${draft.latest_revision}`} base={base} draft={draft} /><ApplicationRevisionEditor key={`edit/${draft.id}/${draft.latest_revision}`} base={`${base}/${encodeURIComponent(draft.id)}`} name={draft.name} expected={draft.latest_revision} onSaved={() => { refresh(); setMessage('New revision saved. Running containers were not changed.'); }} />{!draft.removed_at && instances.state === 'ready' && <ApplicationAdoption key={instances.data?.find((i) => i.application_id === draft.id)?.id ?? draft.id} applicationName={draft.name} base={`${base}/${encodeURIComponent(draft.id)}`} org={org} env={env} instance={instances.data?.find((i) => i.application_id === draft.id)} onChanged={refresh} />}{!draft.removed_at && instances.state === 'ready' && instances.data?.filter((i) => i.application_id === draft.id).map((i) => <div key={i.id}><ApplicationMapping base={`${base}/${encodeURIComponent(draft.id)}`} instanceID={i.id} /><ApplicationComparison base={`${base}/${encodeURIComponent(draft.id)}`} instanceID={i.id} /><ApplicationPreflight org={org} base={`${base}/${encodeURIComponent(draft.id)}`} instanceID={i.id} /><ApplicationDeploymentPlan base={`${base}/${encodeURIComponent(draft.id)}`} instanceID={i.id} latestRevision={draft.latest_revision} instance={i} /></div>)}</>}
      </li>)}
    </ul>}
