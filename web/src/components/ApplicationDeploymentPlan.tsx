...
function PlanView({ base, instanceID }: Props) {
  ...
  const current = planned ?? deployments.data?.find(d => d.instance_id === instanceID) ?? null;
  const page = usePagination(current?.plan.services ?? [], base);
  const reload = () => { mapping.reload(); deployments.reload(); };
  const state = mapping.state !== 'ready' ? mapping.state : deployments.state;
  ...
  return <>
    ...
    {state === 'ready' && mismatched && <p role="alert">Adoption changed. Refresh applications before planning.</p>}
    {state === 'ready' && !mismatched && (current ? <>
    ...
    {state === 'ready' && !mismatched && mapping.data && <form className="dr-stack" onSubmit={e => { e.preventDefault(); void plan(); }}>
