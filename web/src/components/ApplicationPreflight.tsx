...
const messages: Record<Blocker, string> = {
  runtime_verification_required: 'Agent runtime inspection and deployment execution are not available yet. Mounts, networking, image platform, secrets and other configuration remain unverified.',
...
type Preflight = { instance_id: string; endpoint_name: string; revision: number; mapping_version: number; received_at: string; executable: boolean; blockers: Blocker[]; services: { name: string; reference: string; image_id: string; container_id: string; blockers: Blocker[] }[] };
export function ApplicationPreflight({ base, instanceID }: { base: string; instanceID: string }) {
  ...
  return <section className="dr-stack" style={{ overflowWrap: 'anywhere' }}>
    ...
    {open && <PreflightView base={base} instanceID={instanceID} />}
  </section>;
}
function PreflightView({ base, instanceID }: { base: string; instanceID: string }) {
  ...
  return <>
    ...
    {resource.state === 'ready' && data && (data.instance_id !== instanceID ? <p role="alert">Adoption changed. Refresh applications before running preflight.</p> : <>
      ...
      <button type="button" className="btn-secondary" onClick={resource.reload}>Refresh preflight</button>
      {page.controls}
      <table className="ky-table ky-responsive-table"><thead><tr><th>Service / target</th><th>Local image</th><th>Preflight findings</th></tr></thead><tbody>{page.rows.map(s => <tr key={s.name}>
        ...
        <td data-label="Preflight findings">{s.blockers.length ? <ul>{s.blockers.map(b => <li key={b}>{messages[b] ?? 'Unknown condition'}</li>)}</ul> : 'No additional inventory findings; runtime verification still required.'}</td>
      </tr>)}</tbody></table>
    </>)}
  </>;
}
