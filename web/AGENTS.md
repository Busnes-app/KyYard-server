...
- Tenant screens read through `useTenantResource` and show exactly one of loading / denied (401, 403) / not found (404) / offline (network failure) / error, with a retry where retrying can help. Writes go through `tenantWrite` on `secureFetch`; `409 last_administrator` and 403/404/409 map to user-facing messages, never raw server errors.
