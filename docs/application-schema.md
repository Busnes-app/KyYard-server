...
- Three phases, so no registry call holds a transaction. Read: mapping (`mapping_version < 1` is `409 mapping_required`), the preflight on the latest revision (stale inventory or moved containers is `409 adoption_changed`), each mapped service's reference, local digest and registry access; `pinned`, `unknown_local` and `not_configured` make no request. Resolve: `Head` per remaining service, at most 4 at once, 60 s overall. Write: refused with `409 adoption_changed` and nothing written when the instance was released or its mapping version, latest revision or `(container, image)` pairs changed since the read; otherwise the instance's rows are replaced (unmapped services drop out) and the check is audited on `<app>/updates` with details `services=N updates=N errors=N`.
