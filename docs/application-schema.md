...
2. **Apply, implemented.** `POST .../deployments/{deployment}/apply` under `application.deploy` takes `{confirm}` on a planned row. Order: read the plan, check the endpoint advertises `deployment.apply` (501 otherwise), check it is connected (409 `endpoint_offline`), then `ApplyDeployment`, which rechecks the row is `planned` and unexpired, `confirm` equals the plan's project, the application is still at the plan's head revision and spec digest, the instance's mapping version and endpoint are unchanged and active, and each replaced container's resource row still exists; it resolves the encrypted values, builds and validates the `DeploymentRequest`, and moves the row `planned → applying` by compare-and-set with a 10-minute deadline. Delivery failure fails the row (`FailDeployment`) and returns 409 `deployment_not_sent`; success is 202. Planning again while a row is `applying` returns `409 deployment_in_progress`; deployments still serialize per instance (`idx_deployments_live`, one `planned` or `applying` row).
