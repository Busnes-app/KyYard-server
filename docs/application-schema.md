...
- `ResolveRegistryAccess` (internal) runs under the calling operation's permission (`image.pull` or `application.deploy`; every other action is refused) and returns the host's row and decrypted credential, `Anonymous` for an unconfigured host when the opt-in is on, or `registry_not_configured`. Callers never return the secret.
