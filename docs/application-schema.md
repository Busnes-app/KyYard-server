...
**Status:** M6 Compose discovery and internal application/revision persistence are implemented. Bounded public draft import and encrypted environment resolution are implemented. Explicit container-snapshot adoption and release are implemented. Deployment plan and apply are implemented end to end (Deploy, below); history, deliberate reapply and application removal are implemented. M7a PR A (registries and the registry client, Registries below) and PR B (update detection, below) are implemented; the pull step with the controlled recreate (PR C) is next. Plan-time live inspection of host configuration (beyond the on-demand preflight modal) remains a later slice.
...
- Deferred: PR C pins a repository digest in a plan, adds the agent `pull` step with the credential carried in the frame, and enforces `registry_not_configured` at preview and pull.

## Update detection (M7a PR B, implemented)

- On demand, no timer (periodic checks are M7b policies), under `application.deploy`, the operation whose plan the result feeds. It changes nothing on the host and mints no plan.
...
- Routes: `GET .../applications/{application}/updates` (`application.read`) returns the cached rows; `POST .../updates/check` (`application.deploy`, CSRF) runs a check; a second check for the same application while one runs is `409 check_in_progress`, answered only after authorization. The application page shows an Updates panel for a mapped instance. No pull or deploy action: PR C adds it.
...
| Update detection | on demand with a cached result, no timer, under `application.deploy` | implemented (PR B) |
