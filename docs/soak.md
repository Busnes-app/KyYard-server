...
| Did retention itself run? | roll-up and prune errors are counted and fail the run. A run longer than the raw window must have produced at least one hourly summary, or the summary half of the policy went untested while every check that reads summaries found none. If the check itself cannot read the roll-up window (the table is gone), that is reported as a failure too, not an aborted run |
