...
| Deployment history | settled rows (`applying`/`planned` excluded) older than 90 days by settle time pruned (`DeploymentHistoryRetention`) except the latest succeeded row at the instance's current and previous revision, a row whose automated validation's rollback is undecided, and any row of a removed application; a removed application (`removed_at`) is deleted with its revisions and deployment history 90 days after removal (`ApplicationRemovedRetention`); every settle, refusal, abandon, sweep and delivery-failure transition writes an audit row by the row's kind (`application.deploy` for an apply row, `application.destroy` for a remove row, refusals included; `system` actor and `outcome=abandoned/swept/not_sent` for the latter three) | 50 revisions per application (oldest non-current pruned); per-deployment step outcomes and new container identities are stored as JSON on the row itself (no separate events table yet), capped at `MaxDeploymentResultStoredBytes` (160 KiB) | 100 per page | "older revisions pruned" note |
...
## Application revision foundation

Implemented admission bounds: 100 applications per organization, 100 immutable revisions per application, 64 KiB per encoded spec, 100 services per spec and 128 environment secret references per service. Refuse further writes at the limit; never prune saved history silently. Explicit administrator draft discard releases capacity and removes that draft’s revisions atomically with its audit record. These values remain subject to the capacity soak gate. Public bounded draft import/list/read/discard is implemented. Imported environment bundles are at most 64 KiB serialized ciphertext input per revision (16 KiB per plaintext value); encrypted values are retained and discarded with their revision. New audit resource fields are sanitized and bounded to 255 UTF-8 bytes on both backends; existing historical rows are preserved.

Adoption adds one instance per application and at most 1,000 immutable container ownership records per endpoint. Ownership is never silently pruned or extended to replacements. Explicit release removes the instance and its associations with an audit record; environment summaries return counts, while endpoint ownership responses include at most 1,000 resource identities.
