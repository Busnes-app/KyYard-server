...
| Registry credential leakage | Encrypted at rest; delivered only inside the authorizing command; sent only to a registry whose host exactly equals the credential's configured host, never attached to a redirected request, and a reference on an unconfigured registry is refused by default (anonymous pulls from unconfigured hosts are an explicit, audited per-organization opt-in that only `registry.manage`, held by organization administrators, can enable), so an operator who may pull but not manage credentials can neither choose the host that receives a credential nor run an image from an arbitrary host; scrubbed from results, events, inspect, previews, diffs, audit and logs; never persisted by the agent | redaction tests per resource API (M4–M7); `TestCredentialOnlySentToConfiguredHost` (M6); protocol section 8 |
