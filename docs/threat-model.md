...
| Impersonated endpoint | Identity bound to the reviewed public key; a rotation never retires that key until an operator acknowledges the new fingerprint, so a copied identity volume cannot take the endpoint over silently; wrong key fails the challenge; tenant binding checked on every command; a second live connection for one endpoint is refused, recorded, and blocks rotation | protocol tests 2, 4, 7, 13 |
...
| Registry credential leakage | Encrypted at rest; delivered only inside the authorizing command; sent only to a registry whose host exactly equals the credential's configured host, never attached to a redirected request, and a reference on an unconfigured registry is pulled anonymously or refused, so an operator who may pull but not manage credentials cannot choose the host that receives one; scrubbed from results, events, inspect, previews, diffs, audit and logs; never persisted by the agent | redaction tests per resource API (M4–M7); `TestCredentialOnlySentToConfiguredHost` (M6); protocol section 8 |
...
| Cloned agent identity volume | Duplicate live connections refused and recorded; rotation blocked after a duplicate and never retires the approved key without acknowledgement; revocation is the remedy | protocol tests 7, 13 |
