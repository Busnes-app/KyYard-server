...
The published server image contains `/app/kyyard-agent`. Remote setup defaults to `ghcr.io/busnes-app/kyyard:latest` and optionally uses a digest-pinned `KY_AGENT_IMAGE` override. Both use default bridge and outbound HTTPS; HTTP-only deployments show setup guidance. No server container inspection or shared server network namespace is needed. The agent prints its public-key fingerprint for approval. Legacy native stdin enrollment and `--enroll-only` remain supported.
