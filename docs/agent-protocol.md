...
4. Rotation: the agent may present a new public key with a signature by the current key over `"kyyard-agent-rotate-v1" || len(old_fingerprint) || old_fingerprint || len(new_public_key) || new_public_key` (`identity.rotate`, raw 32-byte fields). The server records the new key as `pending_review` and keeps the previously approved key valid; both authenticate until an operator with `endpoint.enroll` acknowledges the new fingerprint in the UI, which retires the old key, or revokes the endpoint. Every rotation raises a high-severity endpoint event that the endpoint list surfaces until acknowledged, and both fingerprints are shown side by side. Rotation is refused for an endpoint that has recorded a `duplicate_connection` event until an operator clears it. A rotation therefore never replaces the reviewed fingerprint binding on its own: a copied identity volume cannot lock the real agent out, because the old key keeps working until a human retires it, and the unacknowledged rotation is the alert.
...
7. Rotation while the original key is offline: the original key still authenticates until an operator acknowledges; the endpoint carries an unacknowledged-rotation event and the API exposes both fingerprints; acknowledgement retires the old key; rotation is refused after a `duplicate_connection` event.
...
| Identity | Ed25519 keypair in an agent volume, fingerprint reviewed at approval; rotation keeps the approved key valid until an operator acknowledges the new fingerprint | proposed |
