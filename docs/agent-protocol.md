...
1. An organization or environment administrator requests an enrollment token for one organization, one environment and one runtime type. The server stores only the SHA-256 of the 32-byte token, with a *proposed* 15-minute expiry, and shows it once together with a generated command that mounts a persistent identity volume, enrolls through `docker run --rm -i ... --enroll-only`, then starts the persistent agent with `docker run -d`. The attached enrollment phase is required to deliver stdin; piping a token to a detached `docker run -d -i` does not deliver it. The token is piped to the agent on stdin, never placed in `-e` or a command argument, so `docker inspect` and the process list never carry it; the shell history of the pasting operator does, which the single use and 15-minute life bound.
The published server image also contains `/app/kyyard-agent`. Default enrollment uses the installed local image ID with `--pull never` and the server network namespace; remote HTTPS enrollment uses the explicitly configured digest-pinned `KY_AGENT_IMAGE`. Operators recreate a same-host agent after replacing the server container, preserving the named identity volume (README runbook). The agent prints its own public-key fingerprint for the approval comparison.
