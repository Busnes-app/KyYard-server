...
4. Rotation: the agent may present a new public key with a signature by the current key over `"kyyard-agent-rotate-v1" || old_fingerprint || new_public_key` (`identity.rotate`); the server keeps both valid for one reconnect window (*proposed* 5 minutes) then retires the old key. Rotation is audited and every fingerprint the identity has held is shown in the endpoint's history, so an operator can tell a rotation from an approval. An interrupted rotation leaves the old key valid until the window ends, so a crash mid-rotation cannot strand the agent. Rotation needs no human approval: possession of the identity volume already equals possession of the endpoint, and the defence against a copied volume is the duplicate-connection refusal above plus revocation.
...
7. Rotation interrupted after the new key is stored: old key still authenticates within the window; after it, only the new key.
...
| Identity | Ed25519 keypair in an agent volume, fingerprint reviewed at approval, rotation with overlap window | proposed |
