...
- Real Docker redacted inspection (runtime and authorized HTTP/agent round trip), exec PTY, deployment replacement and browser-protocol/server/agent regressions in the Go job (isolated fixtures, explicit non-root user, first approved connection, resize and exit status).
