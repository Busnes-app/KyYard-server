...
- Real Docker redacted inspection (runtime and authorized HTTP/agent round trip), exec PTY, deployment replacement and removal, the full apply regression (`TestApplyRealDocker`, `KY_TEST_DOCKER_DEPLOY_IMAGE`: server plus a real agent plan/apply a fixture container to `succeeded`, resources rebound, `current_revision` advanced) and browser-protocol/server/agent regressions in the Go job (isolated fixtures, explicit non-root user, first approved connection, resize and exit status).
