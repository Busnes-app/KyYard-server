...
- Real Docker redacted inspection (runtime and authorized HTTP/agent round trip), exec PTY, deployment replacement and removal (`TestRemoveRealDocker`), the full apply-then-remove regression (`TestApplyRealDocker`, `KY_TEST_DOCKER_DEPLOY_IMAGE`: server plus a real agent plan/apply a fixture container to `succeeded`, resources rebound, `current_revision` advanced, then removes the application and asserts the containers are gone, the network remains, the instance is released and the application is marked removed) and browser-protocol/server/agent regressions in the Go job (isolated fixtures, explicit non-root user, first approved connection, resize and exit status).
