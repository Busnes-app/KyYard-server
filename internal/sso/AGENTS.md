...
- UI-configured endpoints require HTTPS with certificate verification. They are administrator-trusted identity infrastructure (private KyIdentity hosts allowed). HTTPS discovery cannot downgrade authorization, token or signing-key endpoints to HTTP. Network calls have deadlines, redirects are refused, profile bodies are capped at 1 MiB, and credentials stay server-side. PKCE S256 is required; provider-specific nonstandard grants/profile formats are not claimed as supported.
- `KySignOnClient.HandleSyncWebhook` verifies HMAC-SHA256 signatures before modifying local user state.
- `HandleSyncWebhook` refuses a `user.created` whose username matches an existing account ignoring case with `ErrUsernameTaken` (named in the error), never linking to or duplicating that account; a racing create is caught by `idx_users_username_lower` the same way. A `user.updated` renaming onto a taken username is the same `ErrUsernameTaken` (from `UpdateUser`'s `ErrAlreadyExists`), so driver text never reaches the caller. Every `ErrUsernameTaken` writes one audit row `auth.sso.refused` (result `denied`, resource `kysignon`, details `username=` plus the name through `protocol.CleanText`, 64 bytes); never the payload or signature.
