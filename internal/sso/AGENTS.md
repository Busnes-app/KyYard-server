...
- `HandleSyncWebhook` refuses a `user.created` whose username matches an existing account ignoring case with `ErrUsernameTaken` (named in the error), never linking to or duplicating that account; a racing create is caught by `idx_users_username_lower` the same way.
