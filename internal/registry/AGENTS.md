...
- `Head` is `Resolve` with `HEAD` on the manifest URL (one shared `manifest` function): same guard, auth flow and budget (two HEADs, one token GET). It reads no body; the digest is `Docker-Content-Digest` (format-checked, equal to a requested digest), the media type the `Content-Type`, `Platforms` nil. Docker Hub counts a manifest GET as a pull and not a HEAD, so PR B polls with `Head` and calls `Resolve` only when the digest changed or platforms are needed.
- The store calls `Head` through `store.DigestResolver` (`CheckImageUpdates`), passing the credential and `allowPrivate` per call; the adapter uses a `Client` whose `Options.AllowPrivate` equals that call's `allowPrivate`, one client per setting.
