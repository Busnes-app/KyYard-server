...
func TestKubernetesStepCodes(t *testing.T) {
	for _, tc := range []struct {
		...
		{"rollout_timeout", "progressing=NewReplicaSetCreated,replicafailure=FailedCreate", true},
		{"rollout_timeout", "replicafailure=exceeded quota", false},
		{"unsupported", "k8s_volume,k8s_restart", true},
		{"pod_security", "", true},
		{"pod_security", "missing", true},
		{"pod_security", "privileged", true},
		{"pod_security", "invalid", true},
		{"pod_security", "baseline", false},
		{"pod_security", "strict", false},
		{"admission_denied", "", true},
		{"admission_denied", "ConfigMap/shop-web-env", true},
		{"admission_denied", "Pod/shop-web", false},
		{"admission_denied", "exceeded quota", false},
	} {
