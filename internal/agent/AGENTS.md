...
- Rotation (`--rotate-every`, default 30 days, 0 disables): at the start of a session past the interval the agent mints a key, persists it as pending before sending `identity.rotate` signed by the current key, and keeps authenticating with the current key. `identity.rotated` from the server, or a connect refused with `key_retired` while a pending key exists, promotes the pending key and reconnects at once; `key_retired` with no pending key is terminal like revocation. A rotation the server declines (`rotation_pending`, `rotation_blocked`, `rotation_refused`) drops the pending key.
