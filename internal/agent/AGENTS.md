...
- Identity lives in `identity.json` under a 0700 directory as a 0600 file written atomically: endpoint ID, Ed25519 private key, the pinned instance fingerprint and the server origin. The enrollment token is read from stdin once and never written anywhere.
- `Enroll` checks the server echoed our fingerprint before saving; `ConnectURL` refuses plaintext to any host that is not loopback; a challenge whose instance fingerprint differs from the pinned one is `ErrInstanceChanged` and stops the loop.
- `Run` reconnects with 1 s → 60 s exponential backoff and ±20 % jitter, heartbeats at the interval the server states (capped below 60 s so nginx defaults hold), sends one inventory snapshot per session with a rising generation, reconnects on `enrollment.approved`, and stops on `identity_revoked`/`enrollment_rejected` or an incompatible version. Cancelling the context closes the socket with `agent_shutdown`.
