...
- `removeImage` refuses an image a container still uses rather than forcing it, which would leave running containers pointing at nothing. The reference rather than the resolved image ID is what gets deleted, because Docker untags a tag and deletes an ID; identity is pinned instead by `Expects.ImageDigest`, re-read immediately before the delete, so a tag that has moved since the decision is `denied` with nothing attempted.
