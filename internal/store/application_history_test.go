...
func TestPlanPriorRevision(t *testing.T) {
	...
	for _, rev := range []int{3, 0} {
		r := planRequest(m)
		r.Revision = rev
		if _, err := ts.PlanDeployment(ctx, a, app.ID, r, nil, imageCheckKey, false); !errors.As(err, &blocked) || !slices.Contains(blocked.Blockers, "service_unmapped") {
	...
	r := planRequest(m)
	r.Revision = 4
	if _, err := ts.PlanDeployment(ctx, a, app.ID, r, nil, imageCheckKey, false); !errors.Is(err, ErrNotFound) {
		t.Fatalf("missing revision: %v", err)
	}
	r.Revision = 2
	d, err := ts.PlanDeployment(ctx, a, app.ID, r, nil, imageCheckKey, false)
	...
	r = planRequest(m)
	r.Revision = 2
	if _, err := ts.PlanDeployment(ctx, a, app.ID, r, nil, imageCheckKey, false); !errors.As(err, &blocked) || !slices.Equal(blocked.Blockers, []string{"revision_services_differ"}) {
...
func TestRemoveApplicationBuildsARemovalAndSettles(t *testing.T) {
	...
	if _, err := ts.PlanDeployment(ctx, a, app.ID, planRequest(m), nil, imageCheckKey, false); !errors.Is(err, ErrNotFound) {
...
func TestRemovalPreconditions(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		"live plan": {func(t *testing.T, st *SQLStore, a TenantAccess, app *Application, _ string, m *ApplicationMapping) RemovalBody {
			if _, err := st.Tenancy().PlanDeployment(ctx, a, app.ID, planRequest(m), nil, imageCheckKey, false); err != nil {
				t.Fatal(err)
			}
			return RemovalBody{InstanceID: m.InstanceID, Confirm: "shop"}
