...
func Open(ctx context.Context, cfg config.DatabaseConfig) (Store, error) {
	...
	switch driver {
	case "sqlite", "sqlite3", "":
		return openSQLite(ctx, cfg.DSN, cfg.SampleCeiling)
	case "postgres", "postgresql", "pgx":
		return openPostgres(ctx, cfg.DSN, cfg.MaxOpenConns, cfg.MaxIdleConns, cfg.ConnMaxLifetime, cfg.SampleCeiling)
	default:
		return nil, fmt.Errorf("unsupported database driver: %q (supported: sqlite, postgres)", cfg.Driver)
	}
}

func openSQLite(ctx context.Context, dsn string, ceiling int) (Store, error) {
	filePath := dsn
	...
	return newSQLStore(ctx, db, "sqlite", ceiling)
}

func openPostgres(ctx context.Context, dsn string, maxOpen, maxIdle int, maxLifetime time.Duration, ceiling int) (Store, error) {
	...
	return newSQLStore(ctx, db, "postgres", ceiling)
}
