...
- `KY_AGENT_IMAGE` is optional and, when set, must be a digest-pinned reference (`registry/repo@sha256:<64 hex>`); a tag is refused at startup because the image runs with the Docker socket. Empty selects the same-host command using the installed server image ID. A configured image selects remote enrollment only when `KY_APP_URL` is HTTPS; its `/app/kyyard-agent` binary is used explicitly.
