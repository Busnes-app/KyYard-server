...
- `KY_AGENT_IMAGE` is optional and, when set, must be a digest-pinned reference (`registry/repo@sha256:<64 hex>`); a tag is refused at startup because the image runs with the Docker socket. Empty uses the official published `ghcr.io/busnes-app/kyyard:latest` image with `--pull always`. Enrollment commands require HTTPS `KY_APP_URL`; HTTP-only installs return setup guidance. The image’s `/app/kyyard-agent` binary is used explicitly.
