...
## Local Contracts
- `KY_DOCKER_SOCKET` defaults to `/var/run/docker.sock`; an explicitly empty value disables the built-in local connection. Compose mounts `KY_DOCKER_SOCKET_PATH` from the host (default `/var/run/docker.sock`) at that container path, with no implicit source-directory creation. Removing the mount is required to remove host Docker authority, independently of disabling the connection.
- `KY_AGENT_IMAGE` is optional and, when set, must be a digest-pinned reference (`registry/repo@sha256:<64 hex>`); a tag is refused at startup because the image runs with the Docker socket. Empty discovers the installed server image’s official digest via the local Docker socket; discovery failure requires an explicit pinned override. Defaults and overrides both use `IsPinnedAgentImage`; no mutable tag is generated. Enrollment commands require HTTPS `KY_APP_URL`; HTTP-only installs return setup guidance. The image’s `/app/kyyard-agent` binary is used explicitly.
